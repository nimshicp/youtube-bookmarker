    ...
    <div className="min-h-screen bg-neutral-950 text-white p-8 relative overflow-hidden">
      ...
      <main className="max-w-7xl mx-auto relative z-10 flex flex-col items-center">
        <div className="w-full flex flex-col items-start mb-12">
          <h1 className="text-4xl font-extrabold tracking-tight mb-6 bg-gradient-to-r from-white to-neutral-400 bg-clip-text text-transparent">
            My Collections
          </h1>
          ...
          <div className="flex gap-3 w-full max-w-md">
            <input
              ...
              className="flex-grow bg-neutral-900/60 border border-neutral-800 rounded-xl px-4 py-3 text-white placeholder-neutral-500 focus:outline-none focus:border-blue-500 backdrop-blur-md shadow-lg"
            />
            <button
              onClick={() => createMutation.mutate(newCollectionName)}
              disabled={!newCollectionName.trim() || createMutation.isPending}
              className="px-6 py-3 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-medium rounded-xl transition-colors shadow-lg shadow-blue-500/20"
            >
              Create
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {collections && collections.map((collection) => (
              <Link
                ...
                key={collection.id} 
                className="group bg-neutral-900/40 border border-neutral-800 rounded-2xl p-6 hover:bg-neutral-800/60 hover:border-neutral-700 transition-all duration-300 backdrop-blur-md hover:shadow-2xl hover:-translate-y-1 flex flex-col items-start"
              >
                ...
                <h3 className="font-semibold text-xl text-white mb-1 group-hover:text-blue-400 transition-colors">
                  {collection.name}
                </h3>
